      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
           <div className="flex glass-pill p-1 rounded-2xl w-full sm:w-auto overflow-x-auto max-w-full no-scrollbar">
             {["Daily", "Weekly", "Monthly", "Yearly"].map(f => (
               <button 
                 key={f}
                 onClick={() => setFilter(f)}
                 className={`flex-1 sm:flex-none px-4 sm:px-5 py-2.5 rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${filter === f ? "bg-orange-600 text-white shadow-lg" : "text-muted hover:text-subheading"}`}
               >
                 {f}
               </button>
             ))}
           ...
           <button 
             ...
             className="w-full sm:w-auto flex items-center justify-center gap-3 bg-orange-500 text-white px-8 py-3.5 rounded-[1.2rem] sm:rounded-[1.5rem] font-black text-xs uppercase tracking-widest shadow-2xl shadow-orange-500/20 hover:bg-orange-600 transition-all active:scale-95 whitespace-nowrap"
           >
          ...
          {filteredTransactions.map((t) => (
            <div key={t._id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 glass-pill rounded-2xl group transition-all hover:bg-white/5 gap-4">
               <div className="flex items-center gap-4">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-orange-500/10 text-orange-500 flex items-center justify-center font-black shadow-inner flex-shrink-0">
               ...
               <div className="flex items-center justify-between sm:justify-end gap-4 sm:gap-8">
                 <span className="font-black text-orange-500 text-base sm:text-lg tracking-tight">
                   -${Number(t.amount).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                 </span>
                 <div className="flex items-center gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                   <button onClick={() => handleEdit(t)} className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl glass-pill flex items-center justify-center text-orange-400 hover:text-orange-300 hover:bg-orange-500/10 transition-colors"><Edit2 size={14} /></button>
                   <button onClick={() => handleDelete(t._id)} className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl glass-pill flex items-center justify-center text-red-500 hover:text-red-400 hover:bg-red-500/10 transition-colors"><Trash2 size={14} /></button>
                 </div>
               </div>
            </div>
          ))}
