      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
           <div className="flex glass-pill p-1 rounded-2xl w-full sm:w-auto">
             {["Daily", "Weekly", "Monthly", "Yearly"].map(f => (
               <button 
                 key={f}
                 onClick={() => setFilter(f)}
                 className={`flex-1 sm:flex-none px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === f ? "bg-teal-600 text-white shadow-lg" : "text-muted hover:text-subheading"}`}
               >
                 {f}
               </button>
             ))}
           ...
           <button 
             ...
             className="w-full sm:w-auto flex items-center justify-center gap-3 bg-teal-600 text-white px-8 py-3.5 rounded-[1.5rem] font-black text-xs uppercase tracking-widest shadow-2xl shadow-teal-600/20 hover:bg-teal-700 transition-all active:scale-95 whitespace-nowrap"
           >
        ...
        <div className="space-y-4">
          {filteredTransactions.map((t) => (
            <div key={t._id} className="flex items-center justify-between p-4 glass-pill rounded-2xl group transition-all hover:bg-white/5">
               <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-teal-500/10 text-teal-500 flex items-center justify-center font-black shadow-inner">
               ...
               <div className="flex items-center gap-8">
                 <span className="font-black text-teal-500 text-lg tracking-tight">
                   +${Number(t.amount).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                 </span>
                 <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                   <button onClick={() => handleEdit(t)} className="w-9 h-9 rounded-xl glass-pill flex items-center justify-center text-teal-400 hover:text-teal-300 hover:bg-teal-500/10 transition-colors"><Edit2 size={16} /></button>
                   <button onClick={() => handleDelete(t._id)} className="w-9 h-9 rounded-xl glass-pill flex items-center justify-center text-orange-400 hover:text-orange-300 hover:bg-orange-500/10 transition-colors"><Trash2 size={16} /></button>
                 </div>
               </div>
            </div>
          ))}
